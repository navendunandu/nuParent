...
const Reminder = () => {
  ...
  const loadReminder = async () => {
    try {
      ...
      const reminderData = querySnapshot.docs.map((doc, index) => ({
        id: doc.id,
        index: index + 1,
        ...doc.data(),
      }))
      ...
      const joinedData = reminderData
        .filter((reminderDataItem) =>
          ageGroupsData.some((ageGroup) => ageGroup.id === reminderDataItem.age)
        )
        .map((reminderDataItem) => {
          const matchingAgeGroup = ageGroupsData.find(
            (ageGroup) => ageGroup.id === reminderDataItem.age
          )
